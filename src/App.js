...
function App() {
  ...
  const removeFromTOur = (id)=>{
    let newTour = destenation.filter((obj)=>{
      return obj.id !== id
    })
    setDestenation(newTour)
      if (destenation.length === 1) {
        document.getElementById('no').style.display = 'none'
        document.getElementById('noo').style.display = 'none'
      }
  }
  const[destenation,setDestenation] = useState(data)
  // const fetchApi = ()=>{
  ...
  return <React.Fragment>
    ...
    <hr id='noo'/>
    {destenation.map((des)=>{
      return  <Tours image={des.image} info={des.info} name={des.name} id={des.id} price={des.price}>
      <button onClick={()=>removeFromTOur(des.id)} className='btn'>Not Intrested</button>
      </Tours>
    })}
